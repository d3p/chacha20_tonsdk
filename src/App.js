import { useEffect } from 'react';
import { TonClient } from "@eversdk/core";
import { libWeb } from "@eversdk/lib-web";

const endpoint = "net.ton.dev"
// eslint-disable-next-line react-hooks/rules-of-hooks
TonClient.useBinaryLibrary(libWeb)

function App () {
  const img = "data:image/png;base64,"
  const img64 = Buffer.from(img, "utf8").toString("base64")
  
  useEffect(() => {
    (async () => {
      const client = new TonClient({ network: { endpoints: [endpoint] } })

      const { phrase } = await client.crypto.mnemonic_from_random({
        dictionary: 1,
        word_count: 12
      })

      console.log("phrase: ", phrase);

      const { secret } = await client.crypto.mnemonic_derive_sign_keys({
        phrase
      })

      console.log("secret: ", secret);

      const result = await client.crypto.chacha20({
        data: img64,
        key: secret,
        nonce: "".slice(0, 24)
      })

      console.log("result ", result);
    })()
  }, [])

  return (
    <div className="App">
      hello
    </div>
  );
}

export default App;
