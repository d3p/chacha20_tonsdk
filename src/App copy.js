import { useEffect } from 'react';
import './App.css';
import { TonClient } from "@eversdk/core";
import { libWeb } from "@eversdk/lib-web";

const endpoint = "net.ton.dev"

function App () {
  // const [keys, setKeys] = useState()

  const img = "data:image/png;base64,"

  // eslint-disable-next-line react-hooks/rules-of-hooks
  TonClient.useBinaryLibrary(libWeb)
  const client = new TonClient({ network: { endpoints: [endpoint] } })

  setTimeout(async () => {
    const { phrase } = await client.crypto.mnemonic_from_random({
      dictionary: 1,
      word_count: 12
    })

    console.log("phrase: ", phrase);

    const { secret } = await client.crypto.mnemonic_derive_sign_keys({
      phrase
    })

    console.log("secret: ", secret);

    // const { bytes } = await client.crypto.generate_random_bytes({
    //   length: 96
    // })

    // console.log("bytes ", bytes);

    const result = await client.crypto.chacha20({
      data: img,
      key: secret,
      nonce: ""
    })

    console.log("result ", result);
  }, 2000);

  // setTimeout(async () => {
  //   const { phrase } = await client.crypto.mnemonic_from_random({
  //     dictionary: 1,
  //     word_count: 12
  //   })
  //   console.log("phrase ", phrase);

  //   const data = await client.crypto.mnemonic_derive_sign_keys({
  //     phrase
  //   })
  //   console.log("data ", data);
  // }, 2000);

  // setTimeout(async () => {
  //   const res = await client.crypto.mnemonic_verify({
  //     phrase: "remove write fury beef bitter asset model on october luggage gun way",
  //     dictionary: 1,
  //     word_count: 12
  //   })
  //   console.log("res ", res);
  // }, 2000);

  // setTimeout(async () => {
  //   const res = await client.crypto.hdkey_xprv_from_mnemonic({
  //     phrase: "remove write fury beef bitter asset model off october luggage gun way",
  //     dictionary: 1,
  //     word_count: 12
  //   })
  //   console.log("res ", res);
  // }, 2000);

  // setTimeout(async () => {
  //   const data = await client.crypto.mnemonic_derive_sign_keys({
  //     phrase: "remove write fury beef bitter asset model off october luggage gun way"
  //   })
  //   console.log("data ", data);
  // }, 2000);

  // useEffect(() => {
  //   (async() => {
  //     // eslint-disable-next-line react-hooks/rules-of-hooks
  //     TonClient.useBinaryLibrary(libWeb)
  //     const client = new TonClient({ network: { endpoints: [endpoint] } })

  //     const { phrase } = await client.crypto.mnemonic_from_random({
  //       dictionary: 1,
  //       word_count: 12
  //     })
  //     console.log("phrase ", phrase);

  //     const data = await client.crypto.mnemonic_derive_sign_keys({
  //       phrase
  //     })
  //     console.log("data ", data);

  //   })()
  // })

  return (
    <div className="App">
      hello
    </div>
  );
}

export default App;
